...
import com.zjh.common.constant.ProductConstant;
import com.zjh.gulimall.product.dao.AttrAttrgroupRelationDao;
import com.zjh.gulimall.product.dao.AttrGroupDao;
...
import com.zjh.gulimall.product.dao.AttrDao;
import com.zjh.gulimall.product.entity.AttrEntity;
import com.zjh.gulimall.product.service.AttrService;
...
//        attrEntity.setAttrName(attr.getAttrName());
        ...
        //2、保存关联关系,如果关联关系的分组为空，则不进行保存关联关系
    ...
    public AttrRespVo getAttrInfo(Long attrId) {
        //要获取不止attrEntity中的数据，还需要获取groupName分组ID+catelogPath分类完整路径
        AttrRespVo respVo = new AttrRespVo();
        AttrEntity attrEntity = this.getById(attrId);
        //将attrEntity数据放到respVo中
        ...
        if (attrEntity.getAttrType() == ProductConstant.AttrEnum.ATTR_TYPE_BASE.getCode()) {
            ...
            if (attrgroupRelation != null) {    //分组Id不为空
                respVo.setAttrGroupId(attrgroupRelation.getAttrGroupId());
                AttrGroupEntity attrGroupEntity = attrGroupDao.selectById(attrgroupRelation.getAttrGroupId());
                if (attrGroupEntity != null) {      //分组信息不为空
...
//                新增
    ...
    public void deleteRelation(AttrGroupRelationVo[] vos) {
        ...
        List<AttrAttrgroupRelationEntity> entities = Arrays.asList(vos).stream().map((item) -> {
            AttrAttrgroupRelationEntity relationEntity = new AttrAttrgroupRelationEntity();
            //将对应属性值从item中复制到AttrAttrgroupRelationEntity中
    ...
    public PageUtils getNoRelationAttr(Map<String, Object> params, Long attrgroupId) {
        ...
        //2.3)、从当前分类的所有属性中移除这些属性；notIn，eq：只去找到base属性而不取sale属性
